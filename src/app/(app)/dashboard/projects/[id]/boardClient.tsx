...
type Task = { id: string; title: string; description?: string | null; status: 'TODO'|'IN_PROGRESS'|'DONE'|'BLOCKED'; priority: number }
...
export default function BoardClient({ projectId }: { projectId: string }) {
  ...
  async function onDragEnd(res: DropResult) {
    ...
    await fetch(`/api/tasks/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: toCol }) })
  }
  ...
  return (
    <div className="grid gap-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={load} className="px-2 py-1 rounded border border-slate-700 text-slate-200 hover:bg-slate-800 text-sm">Refresh</button>
      </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {columns.map(col => (
              <Droppable droppableId={col} type="TASK" key={col}>
                {(provided) => (
                  <div ref={provided.innerRef} {...provided.droppableProps} className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 min-h-[300px]">
