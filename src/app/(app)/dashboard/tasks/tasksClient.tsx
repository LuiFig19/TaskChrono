...
export default function TasksClient() {
  ...
  async function onAddTask(e: React.FormEvent) {
    ...
    setProjects((cur) => {
      const idx = cur.findIndex(p => p.name.toLowerCase() === optimisticProjectName.toLowerCase())
      if (idx >= 0) {
        const copy = [...cur]
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="grid lg:grid-cols-[1.1fr_1fr] gap-6 relative">
        ...
        <div className="pointer-events-none fixed right-4 top-16 z-50 space-y-2">
          {toasts.map(t => (
            <div key={t.id} className="pointer-events-auto rounded-md border border-slate-700 bg-slate-900/95 px-3 py-2 text-sm text-slate-200 shadow-lg">{t.text}</div>
          ))}
        </div>
      {/* Left: controls */}
      <section>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 mb-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <a href="/dashboard/projects" className="text-sm px-2.5 py-1 rounded border border-slate-700 text-slate-200 hover:bg-slate-800">Projects</a>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <label className="sr-only" htmlFor="task-search">Search tasks</label>
              <input id="task-search" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search tasks" className="rounded-md bg-transparent border border-slate-700 px-3 py-1.5 text-slate-200 w-40" />
              <label className="sr-only" htmlFor="status-filter">Status filter</label>
              <select id="status-filter" title="Status filter" aria-label="Status filter" value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value as any)} className="rounded border border-slate-700 bg-slate-900 text-slate-200 px-2 py-1.5">
        ...
        <form onSubmit={onAddTask} className="rounded-xl border border-slate-800 bg-slate-900 p-4 space-y-3">
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-slate-400">Task title</label>
              <input value={title} onChange={(e)=>setTitle(e.target.value)} placeholder="e.g. Redesign website" className="mt-1 w-full rounded-md bg-transparent border border-slate-700 px-3 py-2 text-slate-200" required />
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            <button disabled={submitting} className="px-3 py-2 rounded-md border border-slate-700 text-slate-200 hover:bg-slate-800 disabled:opacity-60">Add Task</button>
