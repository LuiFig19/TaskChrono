        ...
        <aside className="rounded-xl border border-slate-800 bg-slate-900 p-4 h-[70vh] md:sticky md:top-[calc(var(--nav-h,56px)+16px)]">
          ...
          <div className="space-y-1 overflow-y-auto pr-1">
            {teams.length === 0 ? (
              <a
                href="/dashboard/teams/new"
                className="block px-3 py-2 rounded border border-slate-800 hover:bg-slate-800/60"
              >
              ...
              teams.map((t) => (
                <Link
                  key={t.id}
                  href={`/dashboard/teams/${t.id}?tab=overview`}
                  className="block px-3 py-2 rounded hover:bg-slate-800/60 border border-slate-800/0 hover:border-slate-800"
                >
                  {t.name}
                </Link>
              ))
