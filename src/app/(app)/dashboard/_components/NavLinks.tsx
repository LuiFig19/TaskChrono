    ...
    <nav className="hidden md:flex justify-center items-center gap-1 text-[16px] font-medium tracking-wide text-slate-200 whitespace-nowrap">
      {items.map((it) => (
        <a key={it.href} href={it.href} className="group relative px-3 py-1.5 rounded-md">
          <span className={`transition-[filter,color] ${mounted && isActive(it.href) ? 'text-emerald-400 drop-shadow-[0_0_10px_rgba(99,102,241,0.6)]' : 'group-hover:drop-shadow-[0_0_10px_rgba(99,102,241,0.6)]'} ${it.lock ? 'text-slate-400' : ''}`}>
            {it.label}{it.lock ? ' 🔒' : ''}
          </span>
