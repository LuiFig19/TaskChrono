...
function SubmitBtn() {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      data-auth-solid="create"
      disabled={pending}
      className="mt-2 w-full px-5 py-3 rounded-md bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition-colors disabled:opacity-60 shadow-lg shadow-indigo-500/30 border border-indigo-600"
    >
...
export default function RegisterForm({ callbackUrl }: { callbackUrl: string }) {
  const [state, formAction] = (React as any).useActionState<RegisterState, FormData>(
    registerLocalAction as any,
    {},
  );
