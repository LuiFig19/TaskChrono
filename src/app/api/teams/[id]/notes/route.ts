...
export const GET = withErrorHandling(
  async (request: Request, context: { params: Promise<{ id: string }> }) => {
    ...
    const notes = await prisma.teamNote.findMany({
      where: { teamId: id, pinned: pinned ? true : undefined },
...
export const POST = withErrorHandling(
  async (request: Request, context: { params: Promise<{ id: string }> }) => {
    ...
    const note = await prisma.teamNote.create({
      data: { teamId: id, authorId: userId, title, contentMd, pinned: !!body.pinned },
    });
    try {
      ...
      await prisma.teamActivity.create({
        data: {
          ...
          type: 'note.created',
          actorId: userId,
          payload: { noteId: note.id, title, userName: user?.name || user?.email || 'User' } as any,
        },
      });
      broadcastActivity({
        type: 'note.created',
        message: `${user?.name || 'User'} created a note`,
        meta: { teamId: id, noteId: note.id },
      });
    } catch {}
    ...
    try {
      ...
      await prisma.teamActivity.create({
        data: {
          ...
          type: 'note.deleted',
          actorId: userId,
          payload: { noteId, userName: user?.name || user?.email || 'User' } as any,
        },
      });
      broadcastActivity({
        type: 'note.deleted',
        message: `${user?.name || 'User'} deleted a note`,
