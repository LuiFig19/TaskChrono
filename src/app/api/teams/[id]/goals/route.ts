...
export async function GET(_req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const goals = await prisma.teamGoal.findMany({ where: { teamId: id }, orderBy: { updatedAt: 'desc' }, include: { keyResults: true } })
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const goal = await prisma.teamGoal.create({ data: { teamId: id, ownerId: userId, title, description: body.description || null } })
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const role = await getUserTeamRole(userId, id)
  if (!canManageMembers(role)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  const body = await request.json().catch(()=>({})) as { goalId?: string; status?: 'PLANNED'|'IN_PROGRESS'|'AT_RISK'|'COMPLETE'|'PAUSED' }
  const goalId = String(body.goalId||'')
  if (!goalId) return NextResponse.json({ error: 'goalId required' }, { status: 400 })
  await prisma.teamGoal.update({ where: { id: goalId }, data: { status: (body.status as any) || 'COMPLETE' } })
