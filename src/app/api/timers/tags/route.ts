...
export const POST = withErrorHandling(async (req: Request) => {
  ...
  const { timerId, tags } = (await req
    .json()
    .catch(() => ({ timerId: '', tags: [] as string[] }))) as { timerId: string; tags: string[] };
  if (!timerId) return NextResponse.json({ ok: false });
  await prisma.timer.update({
    where: { id: timerId, organizationId, userId },
    data: { tags: tags.map((t) => t.slice(0, 40)).slice(0, 20) },
  });
