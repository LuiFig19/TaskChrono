...
export async function POST(request: Request) {
  ...
  if (!timerId) {
    const timer = await prisma.timer.create({ 
      data: { organizationId, userId, name: requestedName || 'Timer', notes: body.notes?.slice(0, 2000) } 
    })
    timerId = timer.id
    await prisma.timeEntry.create({ 
      data: { organizationId, userId, name: requestedName || 'Timer', timerId: timerId!, startedAt: new Date(), notes: body.notes?.slice(0, 2000) } 
    })
  } else {
    ...
    if (!active) {
      ...
      await prisma.timeEntry.create({ 
        data: { organizationId, userId, name: effectiveName, timerId: timerId!, startedAt: new Date() } 
      })
    }
  }
