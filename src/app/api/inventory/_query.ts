...
export type InventorySortField = 'name' | 'sku' | 'category' | 'supplier' | 'quantity' | 'minQuantity' | 'costCents' | 'priceCents' | 'updatedAt'
...
export function parseInventoryQuery(url: string): InventoryQuery {
	...
	const status = (statusParam === 'in' || statusParam === 'low' || statusParam === 'out') ? statusParam : undefined
	const sortRaw = (p.get('sort') || '').trim() as InventorySortField
	const sort: InventorySortField | undefined = (
		['name','sku','category','supplier','quantity','minQuantity','costCents','priceCents','updatedAt'] as InventorySortField[]
	).includes(sortRaw) ? sortRaw : undefined
...
export function buildOrderBy(query: InventoryQuery): Prisma.InventoryItemOrderByWithRelationInput[] {
