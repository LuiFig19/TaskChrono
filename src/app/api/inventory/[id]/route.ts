...
export const PATCH = withErrorHandling(
  async (request: Request, { params }: { params: { id: string } }) => {
    ...
    const itemOrg = await prisma.inventoryItem.findUnique({
      where: { id: params.id },
      select: { organizationId: true },
    });
    const membership = itemOrg
      ? await prisma.organizationMember.findFirst({
          where: { userId, organizationId: itemOrg.organizationId },
        })
      : null;
    ...
    await prisma.inventoryActivity.create({
      data: {
        organizationId: (await prisma.inventoryItem.findUnique({
          where: { id: params.id },
          select: { organizationId: true },
        }))!.organizationId,
        itemId: params.id,
...
export const DELETE = withErrorHandling(
  async (_request: Request, { params }: { params: { id: string } }) => {
    ...
    const membership = item
      ? await prisma.organizationMember.findFirst({
          where: { userId, organizationId: item.organizationId },
        })
      : null;
    const role = membership?.role as string | undefined;
    if (role !== 'ADMIN' && role !== 'MANAGER' && role !== 'OWNER') {
