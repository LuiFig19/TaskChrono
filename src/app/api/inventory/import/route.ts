...
export async function POST(request: Request) {
  ...
  if (role !== 'ADMIN' && role !== 'MANAGER') {
	...
	for (let idx = 0; idx < rows.length; idx++) {
		...
		if (!name) { errors.push({ line: idx+2, message: 'Missing name' }); continue }
		const sku = (rec['sku'] || name).trim()
		const quantity = Number(rec['quantity'] || '0')
		const minQuantity = Number(rec['minquantity'] || '0')
		if (!Number.isFinite(quantity) || quantity < 0) { errors.push({ line: idx+2, message: 'Invalid quantity' }); continue }
		if (!Number.isFinite(minQuantity) || minQuantity < 0) { errors.push({ line: idx+2, message: 'Invalid minQuantity' }); continue }
	...
	await prisma.inventoryActivity.create({ data: { organizationId, itemId: (await prisma.inventoryItem.findFirst({ where: { organizationId }, select: { id: true } }))!.id, userId, type: 'IMPORT', message: `Imported ${processed} row(s)` }})
