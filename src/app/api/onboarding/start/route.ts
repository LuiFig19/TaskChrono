...
export async function POST(req: Request) {
  try {
    const body = await req.json().catch(() => ({})) as { name?: string; plan?: string; emails?: string }
    ...
    try {
      if (invites.length) {
        ...
        await prisma.task.createMany({ data: invites.map(e => ({ organizationId: org.id, projectId: project.id, title: `INVITE:${e}` })), skipDuplicates: true })
      }
    } catch {}
    ...
    return NextResponse.json({ error: e?.message || 'Failed to start onboarding' }, { status: 500 })
  }
}
