...
export async function createOrganizationAction(
  ...
  const formData = a && (a as FormData).get ? (a as FormData) : (b as FormData);
  const name = String(formData.get('name') || '').trim();
  const plan = String(formData.get('plan') || 'FREE') as
    | 'FREE'
    | 'BUSINESS'
    | 'ENTERPRISE'
    | 'CUSTOM';
  ...
  if (!priceId) {
    return {
      error:
        'Stripe price ID is missing. Set STRIPE_PRICE_BUSINESS/STRIPE_PRICE_ENTERPRISE to a price_ ID (or STRIPE_PRODUCT_* to auto-resolve).',
    };
  }
