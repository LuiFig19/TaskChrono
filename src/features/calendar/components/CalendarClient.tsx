...
export default function CalendarClient({
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    const optimistic: EventDto = {
      ...
      description: JSON.stringify({ category, notes }),
    };
    ...
    try {
      ...
      try {
        document.dispatchEvent(
          new CustomEvent('tc:calendar-changed', { detail: { action: 'created' } }),
        );
      } catch {}
      setTitle('');
      setNotes('');
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 pt-6 pb-6 dashboard-calendar-page">
      <h1 className="text-2xl font-semibold" style={{ margin: 0, padding: 0 }}>
        Calendar
      </h1>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-slate-400 mt-0.5">
                {new Date(cursor).toLocaleString(undefined, { month: 'long', year: 'numeric' })}
              </div>
            </div>
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      document.dispatchEvent(
                        new CustomEvent('tc:calendar-changed', { detail: { action: 'deleted' } }),
                      );
                    } catch {}
                  }}
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-5">
          ...
          <form onSubmit={onCreate} className="mt-4 grid gap-2">
            ...
            <textarea
              ...
              className="px-3 py-2 rounded-md border border-slate-700 bg-slate-900 text-slate-100"
              placeholder="Optional notes (e.g., Employee out)"
            />
            <button
              disabled={loading}
              className="px-3 py-2 rounded-md bg-indigo-600 text-white hover:bg-indigo-700 transition-colors w-fit"
            >
