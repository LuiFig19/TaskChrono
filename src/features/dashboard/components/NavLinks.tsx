    ...
    <nav className="hidden md:flex justify-center items-center gap-1 text-[16px] font-medium tracking-wide text-slate-200 whitespace-nowrap">
      {items.map((it) => (
        <a
          key={it.href}
          href={it.href}
          className="group relative px-3 py-1.5 rounded-md hover:text-white transition-colors duration-300"
        >
          <span
            className={`transition-all duration-300 ease-in-out ${mounted && isActive(it.href) ? 'text-emerald-400 drop-shadow-[0_0_10px_rgba(99,102,241,0.6)]' : 'group-hover:drop-shadow-[0_0_10px_rgba(99,102,241,0.6)]'} ${it.lock ? 'text-slate-400' : ''}`}
          >
            {it.label}
            {it.lock ? ' 🔒' : ''}
          </span>
          <span
            aria-hidden
            className={`pointer-events-none absolute inset-x-3 -bottom-0.5 h-px bg-gradient-to-r from-transparent ${mounted && isActive(it.href) ? 'via-indigo-400 opacity-100' : 'via-indigo-400 opacity-0 group-hover:opacity-100'} to-transparent transition-opacity duration-300`}
          ></span>
        </a>
      ))}
    </nav>
