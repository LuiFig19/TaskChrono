    ...
    <div>
      ...
      >
        <span aria-hidden className="block relative h-3 w-5">
          <span
            className={`absolute left-0 top-0 h-0.5 w-5 transition-transform duration-200 ${open ? 'translate-y-1.5 rotate-45' : ''} dark:bg-slate-200 light:bg-black`}
          ></span>
          <span
            className={`absolute left-0 top-1/2 -translate-y-1/2 h-0.5 w-5 transition-opacity duration-200 ${open ? 'opacity-0' : 'opacity-100'} dark:bg-slate-200 light:bg-black`}
          ></span>
          <span
            className={`absolute left-0 bottom-0 h-0.5 w-5 transition-transform duration-200 ${open ? '-translate-y-1.5 -rotate-45' : ''} dark:bg-slate-200 light:bg-black`}
          ></span>
        </span>
      </button>
      {mounted &&
              ...
              >
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-sm font-medium dark:text-black light:text-black">
                      Quick Actions
                    </div>
                    <button
                    ...
                    >
                      ...
                      >
                        <span
                          className={`absolute left-0 top-0 h-0.5 w-5 transition-transform duration-200 translate-y-1.5 rotate-45 dark:bg-slate-200 light:bg-black`}
                        ></span>
                        <span
                          className={`absolute left-0 bottom-0 h-0.5 w-5 transition-transform duration-200 -translate-y-1.5 -rotate-45 dark:bg-slate-200 light:bg-black`}
                        ></span>
                      </motion.span>
                    </button>
                  </div>
                  ...
                  <nav
                    className="grid text-sm dark:text-slate-200 light:text-black"
                    data-menu-items
                    style={{ color: 'black' }}
                  >
                    <Link
                      ...
                      data-menu-item
                      className="group px-3 py-2 rounded transition-colors dark:hover:bg-slate-800 light:text-black !font-medium"
                      onClick={() => setOpen(false)}
                    >
                      <span
                        style={{ color: 'black' }}
                    ...
                    <a
                      ...
                      onClick={async (e) => {
                        ...
                        const ok = window.confirm(
                          'Sign out and return to the TaskChrono landing page?',
                        );
                        if (!ok) return;
                        try {
                    ...
                    >
                      <span style={{ color: 'black' }}>
                        Sign Out{' '}
                        <span className="text-xs light:text-black" style={{ color: 'black' }}>
                          ({userEmail || ''})
                        </span>
                      </span>
                    </a>
                ...
                <div className="absolute inset-0 grid place-items-center p-4">
                  ...
                  >
                    ...
                    <div className="grid gap-2 max-h-[360px] overflow-y-auto">
                      ...
                      {orgs.length === 0 && (
                        <div className="text-sm text-slate-400">No other workspaces found.</div>
                      )}
                    </div>
