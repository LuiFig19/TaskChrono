...
export default function CompanySubtext() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch('/api/org/active', { cache: 'no-store', credentials: 'include' });
        if (res?.ok) {
    ...
    function onColor(e: any) {
      if (e?.detail?.color) setColor(e.detail.color);
    }
    function onName(e: any) {
      if (typeof e?.detail?.name === 'string') setName(e.detail.name);
    }
    ...
    return () => {
      ignore = true;
      window.removeEventListener('tc:brand-color' as any, onColor as EventListener);
      window.removeEventListener('tc:brand-name' as any, onName as EventListener);
    };
  }, []);
