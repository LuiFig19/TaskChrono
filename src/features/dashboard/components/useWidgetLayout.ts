...
export function useWidgetLayout(defaultLayout: RglItem[], dashboard: string = 'main') {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/layout?dashboard=${encodeURIComponent(dashboard)}`, {
          cache: 'no-store',
          credentials: 'include',
        });
        if (res.ok) {
          ...
          if (alive)
            setLayout(
              Array.isArray(data.layout) && data.layout.length
                ? data.layout
                : safeParseLocal(key) || defaultLayout,
            );
        } else {
          const ls = safeParseLocal(key) as RglItem[] | null;
  ...
  const put = useCallback(
    async (next: RglItem[]) => {
      try {
        await fetch(`/api/layout?dashboard=${encodeURIComponent(dashboard)}`, {
          method: 'PUT',
  ...
  const reset = useCallback(async () => {
    try {
      await fetch(`/api/layout?dashboard=${encodeURIComponent(dashboard)}`, { method: 'DELETE' });
    } catch {}
    try {
...
function safeParseLocal(key: string): any {
  try {
