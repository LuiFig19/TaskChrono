...
import type { Plan } from '@/lib/widget-config';
...
export default function RadialActions({ plan }: { plan: Plan }) {
  ...
  function goTimers() {
    window.location.href = '/dashboard/timers';
  }
  function goFiles() {
  ...
  const baseBtn =
    'h-10 px-4 inline-flex items-center justify-center gap-2 rounded-full font-medium leading-none shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30 transition-colors whitespace-nowrap';
      ...
      >
        ...
        <div className="relative z-[100000]">
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setSubmenu((s) => (s === 'widgets' ? null : 'widgets'));
            }}
            title="Add Widget"
          >
          ...
          <div
            className={`absolute left-0 top-full mt-2 grid gap-2 z-[100000] transform transition-all duration-200 ${submenu === 'widgets' ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-1 pointer-events-none'}`}
          >
            <AddWidgetPanel
              plan={plan}
              onClose={() => {
        ...
        <div className="relative z-[100000]">
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setSubmenu((s) => (s === 'theme' ? null : 'theme'));
            }}
            title="Theme"
          >
          ...
          <div
            className={`absolute left-0 top-full mt-2 grid gap-2 z-[100000] transform transition-all duration-200 ${submenu === 'theme' ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-1 pointer-events-none'}`}
          >
            <button
              className={`${baseBtn} h-9 rounded-full bg-slate-800 hover:bg-slate-700 text-white border border-slate-600`}
              onClick={() => {
            ...
            <button
              className={`${baseBtn} h-9 rounded-full bg-white hover:bg-slate-100 text-slate-900 border border-slate-300`}
              onClick={() => {
