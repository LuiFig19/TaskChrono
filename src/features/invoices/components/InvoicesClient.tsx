...
function InvoiceStatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded border text-xs ${map[status] || ''}`}>{status}</span>
  );
}
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        ...
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            await createInvoice(new FormData(e.currentTarget));
            (e.currentTarget as HTMLFormElement).reset();
          }}
          className="mt-3 grid gap-2 text-sm"
        >
          <label className="sr-only" htmlFor="clientName">
            Client
          </label>
          <input
            id="clientName"
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="sr-only" htmlFor="issueDate">
              Issue Date
            </label>
            <input
              id="issueDate"
          ...
          />
          <div className="grid grid-cols-2 gap-2">
            <label className="text-slate-300 text-xs" htmlFor="status">
              Status
            </label>
            <label className="text-slate-300 text-xs" htmlFor="recurrence">
              Recurrence
            </label>
            <select
              id="status"
          ...
          <label className="text-slate-300 text-xs" htmlFor="notes">
            Notes
          </label>
          <textarea
            id="notes"
          ...
          />
          <button className="mt-2 px-3 py-2 rounded-md bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">
            Add Invoice
          </button>
        </form>
      </div>
      <div className="space-y-3">
        <div className="flex flex-wrap gap-2">
          <input
            ...
            className="px-3 py-2 rounded-md border border-slate-700 bg-slate-900 text-slate-100 text-sm"
          />
                ...
                return (
                  <tr key={i.id} className="hover:bg-slate-900">
                    ...
                    <td className="px-3 py-2">
                      {issued ? new Date(issued as any).toLocaleDateString() : '-'}
                    </td>
                    <td className="px-3 py-2">
                      {due ? new Date(due as any).toLocaleDateString() : '-'}
                    </td>
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        onClick={() => updateStatus(i.id, i.status === 'PAID' ? 'SENT' : 'PAID')}
                        className="mr-2 px-2 py-1 rounded border border-slate-700 hover:bg-slate-800"
                      >
                        {i.status === 'PAID' ? 'Mark Unpaid' : 'Mark Paid'}
                      </button>
                      <button
                        onClick={() => remove(i.id)}
                        className="px-2 py-1 rounded border border-rose-700 text-rose-300 hover:bg-rose-900/30"
                      >
                        Delete
                      </button>
                    </td>
