...
export default function SettingsClient({ isAdmin, organizationId, plan }: Props) {
  ...
  }>({ team: true, billing: true, integrations: true });
    ...
    async function load() {
      try {
        const [teamRes, orgRes, listRes] = await Promise.all([
          ...
          organizationId
            ? fetch(`/api/org/${organizationId}`, { cache: 'no-store' })
    ...
    try {
      const res = await fetch('/api/company/update', {
        ...
        body: (() => {
          ...
          f.set('id', organizationId);
          f.set('name', orgName.trim());
          f.set('brandColor', brandColor);
          return f;
        })(),
      });
      ...
      notify('Failed to save company settings');
    }
  }

  // Instant broadcast to header
  React.useEffect(() => {
    window.dispatchEvent(new CustomEvent('tc:brand-color', { detail: { color: brandColor } }));
  }, [brandColor]);
  React.useEffect(() => {
    window.dispatchEvent(new CustomEvent('tc:brand-name', { detail: { name: orgName } }));
  }, [orgName]);
  ...
  async function invite() {
    ...
    try {
      ...
      const body = encodeURIComponent(
        `You have been invited to join our TaskChrono workspace. Click Continue to accept: ${data?.acceptUrl || ''}`,
      );
    ...
    <div className="max-w-screen-md mx-auto px-4 py-6">
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
          <div>
            ...
            <input
              ...
              className="mt-1 w-full px-3 py-2 rounded-md border border-slate-700 bg-slate-950 text-slate-100"
            />
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setWheelOpen(true)}
              className="h-10 px-4 rounded-full border border-slate-700 bg-slate-800 text-slate-200 hover:bg-slate-700"
            >
              Pick Color
            </button>
            <div
            ...
            <button
              onClick={saveOrgName}
              className="h-10 shrink-0 px-4 rounded-md bg-indigo-600 text-white hover:bg-indigo-700"
            >
              Save
            </button>
          </div>
      ...
      <section className="mt-6 rounded-lg border border-slate-800 bg-slate-900/60">
        <header className="flex items-center justify-between px-4 py-3">
          ...
          <button
            ...
            onClick={() => setOpen((o) => ({ ...o, team: !o.team }))}
            className="px-2 py-1 text-sm rounded border border-rose-600/30 text-rose-300 hover:bg-rose-900/20"
          >
            {open.team ? 'Hide' : 'Show'}
          </button>
        </header>
            ...
            {isAdmin && (
              <div className="flex flex-wrap items-center gap-2">
                <input
                  ...
                  className="border border-slate-700 bg-transparent px-3 py-2 rounded text-slate-200"
                />
                <button
                  disabled={loading}
                  onClick={invite}
                  className="px-3 py-2 rounded border border-slate-700 hover:bg-slate-800 disabled:opacity-60"
                >
                  Invite
                </button>
                <button
                  disabled={loading || !email}
                  onClick={() => removeByEmail(email)}
                  className="px-3 py-2 rounded border border-slate-700 hover:bg-slate-800 disabled:opacity-60"
                >
                  Remove
                </button>
                  ...
                  <div key={m.id} className="px-3 py-2 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="rounded border border-slate-700 bg-slate-900 text-slate-200 px-2 py-1 text-sm"
                      >
                      ...
                      <button
                        onClick={() => m.email && removeByEmail(m.email)}
          ...
          <div className="px-4 pb-4 grid gap-2">
            ...
            <div className="flex gap-2 mt-1">
              <button
                ...
                className="px-3 py-2 rounded bg-emerald-600 hover:bg-emerald-700 text-white border border-emerald-500"
              >
                Manage Subscription
              </button>
              {plan === 'FREE' && (
                <button
                  ...
                  className="px-3 py-2 rounded border border-indigo-600 text-indigo-300 hover:bg-slate-800"
                >
                  Upgrade Plan
                </button>
              )}
      ...
      <section className="mt-6 rounded-lg border border-slate-800 bg-slate-900/60">
        <header className="flex items-center justify-between px-4 py-3">
          ...
          <button
            data-settings-toggle
            onClick={() => setOpen((o) => ({ ...o, integrations: !o.integrations }))}
            className="px-2 py-1 text-sm rounded border border-rose-600/30 text-rose-300 hover:bg-rose-900/20"
          >
          ...
          <div className="px-4 pb-4 grid gap-4">
            <div className="rounded border border-slate-800 p-3">
              ...
              <button
                ...
                className="mt-2 px-3 py-2 rounded bg-rose-600 hover:bg-rose-700 text-white border border-rose-500"
              >
                Connect to Slack
              </button>
            </div>
...
function ColorWheelModal({
  ...
  const start = React.useMemo(
    () => hexToHsl(initialColor) || { h: 210, s: 90, l: 60 },
    [initialColor],
  );
      ...
      <div className="absolute inset-0 grid place-items-center p-4">
        <div className="w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-950 p-4 shadow-2xl">
          ...
          <div className="mt-4 grid gap-3">
            <label className="sr-only" htmlFor="cw-hue">
              Hue
            </label>
            <input
              id="cw-hue"
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                placeholder="#RRGGBB"
                className="px-3 py-2 rounded-md border border-slate-700 bg-slate-900 text-slate-100 font-mono"
              />
            </div>
          </div>
          <div className="mt-4 flex justify-end gap-2">
            <button
              onClick={onClose}
              className="px-3 py-2 rounded border border-slate-700 hover:bg-slate-800"
            >
              Done
            </button>
          </div>
          ...
          <div className="mt-3 grid gap-2 max-h-[360px] overflow-y-auto">
            {orgs.map((o) => (
              <button
                ...
                className="text-left px-3 py-2 rounded-md border border-slate-700 hover:bg-slate-800 flex items-center justify-between"
              >
                <div>
            ...
            {orgs.length === 0 && (
              <div className="text-sm text-slate-400">
                You only belong to your current workspace.
              </div>
            )}
          </div>
          <div className="mt-4 flex justify-end gap-2">
