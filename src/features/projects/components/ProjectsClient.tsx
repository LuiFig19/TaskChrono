...
function CreateProjectForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) {
      ...
      if (/free tier/i.test(text)) {
        setLimitMsg(
          'You have reached the Free plan limit for projects. Upgrade your plan to add more projects.',
        );
        setLimitOpen(true);
      } else {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="w-full rounded-xl border border-slate-800 bg-slate-900/80 p-2 flex flex-wrap items-center gap-2"
    >
      <div className="flex items-center gap-2">
        <span aria-hidden>📁</span>
        <label className="sr-only" htmlFor="proj-name">
          Project name
        </label>
        <input
          id="proj-name"
          ...
          className="rounded-md bg-transparent border border-slate-700 px-3 py-2 text-slate-200"
          required
        />
      </div>
      <div className="flex items-center gap-2">
        <span aria-hidden>📝</span>
        <label className="sr-only" htmlFor="proj-desc">
          Project description
        </label>
        <input
          id="proj-desc"
          ...
          className="rounded-md bg-transparent border border-slate-700 px-3 py-2 text-slate-200 w-56"
        />
      </div>
      <div className="flex items-center gap-2">
        <span aria-hidden>⚑</span>
        <label className="sr-only" htmlFor="proj-status">
          Status
        </label>
        <select
          id="proj-status"
          name="status"
          className="rounded-md bg-slate-900 border border-slate-700 px-2 py-2 text-slate-200"
        >
      ...
      <div className="flex items-center gap-2">
        <span aria-hidden>💰</span>
        <label className="sr-only" htmlFor="proj-budget">
          Budget
        </label>
        <input
          id="proj-budget"
          value={budgetDisplay}
          onChange={(e) => {
            const f = formatUsd(e.target.value);
            setBudgetDisplay(f.display);
            setBudgetNumber(f.value);
          }}
          inputMode="decimal"
          placeholder="Budget ($)"
      ...
      <button className="px-3 py-2 rounded-md bg-emerald-600 text-white hover:bg-emerald-500 border border-emerald-700/50 light:bg-emerald-600 light:text-white light:hover:bg-emerald-500">
        Create
      </button>
      {limitOpen && (
          ...
          <div className="absolute inset-0 grid place-items-center p-4">
            <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-950 to-slate-900 p-5 shadow-2xl">
              ...
              <div className="text-sm text-slate-300 mt-1">
                {limitMsg || 'Free tier allows up to 5 projects.'}
              </div>
...
function useProjectsData() {
  ...
  const shown = projects
    ...
    .sort((a, b) =>
      sort === 'updated' ? new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime() : 0,
    );
...
function TrashButton({ onClick }: { onClick: (e: React.MouseEvent) => void }) {
  return (
    <button
      ...
      className="p-1.5 rounded border border-rose-700 text-rose-300 hover:bg-rose-900/30"
    >
      <svg
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      {shown.map((p) => (
        <a
          ...
          data-proj-card
          className="group rounded-xl border border-slate-800 bg-slate-900 p-4 hover:bg-slate-900/80 transition-transform hover:-translate-y-0.5"
        >
              ...
              <button
                onClick={(e) => {
                  ...
                  setEditing({
                    id: p.id,
                    name: cur?.name || '',
                    status: cur?.status || 'PLANNING',
                    budgetCents: cur?.budgetCents,
                  });
                  setEditName(cur?.name || '');
                  setEditStatus(cur?.status || 'PLANNING');
                  setEditBudget(
                    cur?.budgetCents ? String(Math.round(Number(cur.budgetCents) / 100)) : '',
                  );
                }}
          ...
          <div className="text-slate-400 text-sm mt-1 line-clamp-2 min-h-[2.5rem]">
            {(p as any).description || 'No description'}
          </div>
          <div className="mt-3 h-1.5 bg-slate-200 rounded overflow-hidden">
            {(() => {
              const pct = Math.min(
                100,
                Math.round((((p as any).doneCount || 0) / Math.max(1, (p as any).taskCount)) * 100),
              );
              ...
              return (
                <div
                  ...
                  style={{
                    width: `${pct}%`,
                    backgroundImage: bg,
                    boxShadow: `0 0 6px hsla(${hue}, 90%, 45%, 0.35)`,
                  }}
                />
              );
            })()}
          ...
          <div className="text-xs text-slate-500 mt-2 flex items-center justify-between">
            ...
            <span className="truncate ml-2">
              Budget: {formatUsdCents((p as any).budgetCents)} · Updated{' '}
        ...
        <div className="fixed inset-0 z-[100000]">
          ...
          <div className="absolute inset-0 grid place-items-center p-4">
            <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-950 to-slate-900 p-5 shadow-2xl">
              ...
              <div className="text-sm text-slate-300 mt-1">Update the project’s details.</div>
              <form
                onSubmit={async (e) => {
                  e.preventDefault();
                  setBusy(true);
              ...
              >
                ...
                <label className="grid gap-1 text-sm">
                  ...
                  <select
                    ...
                    className="px-3 py-2 rounded-md bg-slate-950 border border-slate-700 text-slate-200"
                  >
                    {['PLANNING', 'ACTIVE', 'ON_HOLD', 'COMPLETED', 'CANCELLED'].map((s) => (
                      <option key={s} value={s}>
                        {s.replace('_', ' ')}
        ...
        <div className="fixed inset-0 z-[100000]">
          ...
          <div className="absolute inset-0 grid place-items-center p-4">
            <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-950 to-slate-900 p-5 shadow-2xl">
              ...
              <div className="text-sm text-slate-300 mt-1">
                This will remove the project and its tasks. This action cannot be undone.
              </div>
              <div className="mt-5 flex items-center justify-end gap-2">
                <button
                  type="button"
...
function ProjectsToolbar({
  ...
  view: 'cards' | 'kanban' | 'gantt';
  setView: (v: 'cards' | 'kanban' | 'gantt') => void;
}) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="rounded-md bg-transparent border border-slate-700 px-3 py-2 text-slate-200"
          />
          <label className="sr-only" htmlFor="proj-filter-status">
            Filter status
          </label>
          <select
      ...
      {columns.map((col) => (
        <div key={col} className="rounded-xl border border-slate-800 bg-slate-900/70 p-2">
          ...
          <div className="grid gap-2">
            {shown
              .filter((p) => (p as any).status === col)
              .map((p) => (
                <a
                  key={p.id}
                  href={`/dashboard/projects/${p.id}`}
                  data-proj-card-kanban
                  className="rounded-lg border border-slate-800 bg-slate-900 p-3 hover:bg-slate-900/80"
                >
                  <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-slate-500 mt-1">
                    Budget: {formatUsdCents((p as any).budgetCents)}
                  </div>
                </a>
              ))}
          </div>
