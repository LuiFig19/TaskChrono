...
export default function TimersClient({ userId, initialEntries, initialTimers }: Props) {
  ...
  const [filter, setFilter] = useState<'all' | 'active' | 'paused' | 'ended' | 'today' | 'week'>(
    'all',
  );
  ...
  useEffect(() => {
    ...
    fetch('/api/socket', { credentials: 'include' }).catch(() => {});
    const s = ioClient('/', { path: '/ws', transports: ['websocket', 'polling'] });
    socketRef.current = s;
    s.on('connect', () => {
  ...
  const refresh = useCallback(async () => {
    const res = await fetch('/api/timers/list', { cache: 'no-store', credentials: 'include' });
    if (!res.ok) return;
  ...
  const rows: Row[] = useMemo(() => {
    ...
    for (const e of entries) {
      ...
      if (e.endedAt) {
        ...
        b.activeStart =
          !b.activeStart || new Date(e.startedAt) > new Date(b.activeStart)
            ? e.startedAt
            : b.activeStart;
      }
      b.lastStart =
        !b.lastStart || new Date(e.startedAt) > new Date(b.lastStart) ? e.startedAt : b.lastStart;
    }
  ...
  const filteredRows = useMemo(() => {
    ...
    if (sort === 'recent')
      r.sort((a, b) => new Date(b.startedAt || 0).getTime() - new Date(a.startedAt || 0).getTime());
    if (sort === 'oldest')
      r.sort((a, b) => new Date(a.startedAt || 0).getTime() - new Date(b.startedAt || 0).getTime());
  ...
  const totalEndedMin = useMemo(
    () =>
      filteredRows
        .filter((r) => r.status === 'ended')
        .reduce((acc, r) => acc + (r.durationMin || 0), 0),
    [filteredRows],
  );
  ...
  const pause = useCallback(async (timerId: string | null) => {
    ...
    setEntries((prev) => {
      ...
      for (let i = 0; i < copy.length; i++) {
        const e = copy[i];
        if (e.timerId === timerId && !e.endedAt) {
          idx = i;
          break;
        }
      }
      if (idx >= 0) {
        ...
        copy[idx] = {
          ...
          durationMin: (cur.durationMin || 0) + minutes,
        };
      }
      return copy;
    });
    await fetch('/api/timers/stop', {
      method: 'POST',
      credentials: 'include',
      body: timerId ? new URLSearchParams({ timerId }) : (undefined as any),
    });
  }, []);
  ...
  const resume = useCallback(async (timerId: string | null, name: string) => {
    ...
    await fetch('/api/timers/start', {
      ...
      credentials: 'include',
      body: JSON.stringify({ timerId, name }),
    });
  }, []);

  const end = useCallback(
    async (timerId: string | null) => {
      if (!timerId) return;
      const prevTimers = timers;
      setTimers((prev) =>
        prev.map((t) => (t.id === timerId ? { ...t, finalizedAt: new Date().toISOString() } : t)),
      );
      try {
        const res = await fetch('/api/timers/end', {
          method: 'POST',
          body: new URLSearchParams({ timerId }),
        });
  ...
  const removeTimer = useCallback(async (timerId: string) => {
    ...
    await fetch('/api/timers/remove', { method: 'POST', body, credentials: 'include' });
  }, []);

  const updateTags = useCallback(async (timerId: string, tags: string[]) => {
    await fetch('/api/timers/tags', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ timerId, tags }),
    });
  }, []);
    ...
    <div className="max-w-screen-2xl mx-auto px-4 pt-4 pb-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm text-slate-400" htmlFor="tc-filter">
            Filter:
          </label>
          <select
            id="tc-filter"
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm text-slate-400" htmlFor="tc-tag">
            Tag:
          </label>
          <select
            id="tc-tag"
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm text-slate-400" htmlFor="tc-sort">
            Sort:
          </label>
          <select
            id="tc-sort"
        ...
        <form method="post" action="/api/timers/export" className="ml-auto flex gap-2">
          ...
          <button
            ...
            value="csv"
            data-export="csv"
            className="tc-export px-3 py-2 rounded-md border border-slate-700 text-slate-200 hover:bg-slate-800"
          >
            CSV
          </button>
          <button
                ...
                <tr key={row.timerId} className="hover:bg-slate-900">
                  ...
                  <td className="px-3 py-2">
                    {isActive ? (
                      <>
                        <button
                          className="px-2 py-1 rounded-md bg-amber-500 text-white hover:bg-amber-600 active:scale-[0.98]"
                          onClick={() => pause(row.timerId)}
                        >
                          Pause
                        </button>
                        <button
                          className="ml-2 px-2 py-1 rounded-md bg-rose-600 text-white hover:bg-rose-700 active:scale-[0.98]"
                          onClick={() => openEndModal(row.timerId, row.name)}
                        >
                          End
                        </button>
                      </>
                      ...
                      !isEnded && (
                        <button
                          className="px-2 py-1 rounded-md bg-emerald-600 text-white hover:bg-emerald-700 active:scale-[0.98]"
                          onClick={() => resume(row.timerId, row.name)}
                        >
                          Start
                        </button>
                      )
                    ...
                    <button
                      className="ml-2 px-2 py-1 rounded-md border border-slate-700 text-slate-300 hover:bg-slate-800 active:scale-[0.98]"
                      onClick={() => setConfirmDeleteId(row.timerId)}
                      title="Delete timer"
                    >
                      🗑️
                    </button>
                  ...
                  <td className="px-3 py-2">
                    <TagBadges
                      ...
                      onChange={(next) => {
                        setTimers((prev) =>
                          prev.map((tt) => (tt.id === row.timerId ? { ...tt, tags: next } : tt)),
                        );
        ...
        <div className="fixed inset-0 z-[100000]">
          ...
          <div className="absolute inset-0 grid place-items-center p-4">
            <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-950 to-slate-900 p-5 shadow-2xl">
              ...
              <div className="text-sm text-slate-400 mt-1">
                Are you sure you want to end “{endTarget.name}”? This will finalize and record its
                time.
              </div>
...
function TagBadges({ value, onChange }: { value: string[]; onChange: (v: string[]) => void }) {
  ...
  const commit = useCallback(
    (next: string[]) => onChange(Array.from(new Set(next.map((t) => t.trim()).filter(Boolean)))),
    [onChange],
  );
  return (
      ...
      {inputOpen ? (
        ...
        >
          <input
            ...
            className="px-2 py-1 rounded-md border border-slate-700 bg-slate-900 text-slate-200 w-28"
            placeholder="tag"
            autoFocus
          />
          <button
        ...
        <button
          className="px-2 py-1 rounded-md border border-slate-700 text-slate-300 hover:bg-slate-800"
...
function Analytics({
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) => {
      ...
      if (filter === 'ended' || filter === 'paused' || filter === 'active' || filter === 'all')
        return true;
      return true;
    });
  ...
  const weeklyStacked = useMemo(() => {
    ...
    const top = Array.from(labelTotals.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 4)
      .map(([k]) => k);
    ...
    const lastWeekMinutes =
      entries
        .filter(
          (e) =>
            e.endedAt &&
            new Date(e.startedAt) >= lastWeekStart &&
            new Date(e.startedAt) <= lastWeekEnd,
        )
        .reduce((acc, e) => acc + (e.durationMin || 0), 0) / 60;
    ...
    const leastLabel =
      days[leastIdx]?.date.toLocaleDateString(undefined, { weekday: 'short' }) || '';
    return {
      data,
      keys: [...top, 'Other'].filter(Boolean),
      totalWeek,
      lastWeekHours: lastWeekMinutes,
      most,
      least,
      mostLabel,
      leastLabel,
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div
        data-analytics-card
        className="rounded-md border border-slate-800 p-3 bg-slate-950 max-h-[480px] overflow-y-auto"
      >
        <div className="flex items-center justify-between mb-2">
          <div className="text-slate-200">Productivity breakdown</div>
          <div className="text-xs text-slate-300 flex items-center gap-1">
            <button
              className={`px-2 py-1 rounded-md ${mode === 'projects' ? 'bg-slate-800 text-white' : 'text-slate-300 border border-slate-700'}`}
              onClick={() => setMode('projects')}
            >
              Projects
            </button>
            <button
              className={`px-2 py-1 rounded-md ${mode === 'tags' ? 'bg-slate-800 text-white' : 'text-slate-300 border border-slate-700'}`}
          ...
          {breakdown.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart margin={{ top: 8, right: 8, bottom: 36, left: 8 }}>
                <Pie
                  data={breakdown.map((d, i) => ({ ...d, value: d.minutes }))}
                  dataKey="value"
                  nameKey="label"
                  innerRadius={60}
                  outerRadius={95}
                ...
                <Tooltip
                  ...
                  formatter={(v: any) =>
                    `${formatDuration(Number(v))} (${((Number(v) / totalMinutes) * 100 || 0).toFixed(0)}%)`
                  }
                />
              </PieChart>
            </ResponsiveContainer>
        ...
        <div className="mt-3 space-y-1 text-sm">
          {top3.map((t, i) => (
            <div key={t.label} className="flex items-center justify-between text-slate-300">
              ...
              <div>
                {formatDuration(t.minutes)} ({((t.minutes / totalMinutes) * 100 || 0).toFixed(0)}%)
              </div>
            </div>
          ))}
        </div>
      ...
      >
        ...
        <div className="mt-3 text-sm text-slate-300">
          You tracked{' '}
          <span className="text-white font-medium">{weeklyStacked.totalWeek.toFixed(1)}h</span> this
          week
          {Number.isFinite(weeklyStacked.lastWeekHours) && (
            <>
              ,&nbsp;{weeklyStacked.totalWeek - weeklyStacked.lastWeekHours >= 0 ? '+' : ''}
              {(weeklyStacked.totalWeek - weeklyStacked.lastWeekHours).toFixed(1)}h vs last week
            </>
          )}
          .
          <div className="mt-1">
            Best: {weeklyStacked.mostLabel} {weeklyStacked.most.toFixed(1)}h • Low:{' '}
            {weeklyStacked.leastLabel} {weeklyStacked.least.toFixed(1)}h
          </div>
        </div>
      </div>
    </div>
