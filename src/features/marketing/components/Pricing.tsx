        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {tiers.map((tier, i) => (
            <m.div
              ...
              className={`pricing-card relative rounded-2xl border p-6 bg-white/60 backdrop-blur-sm ${tier.highlight ? 'pricing-card--highlight ring-2 ring-indigo-500 shadow-xl' : 'shadow-sm'}`}
            >
              {tier.highlight && (
              ...
              <div className="mt-6">
                <Link
                  href={tier.cta.href}
                  className={`block text-center px-4 py-2 rounded-lg transition-colors ${tier.highlight ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'border hover:bg-gray-50'}`}
                >
                  {tier.cta.label}
                </Link>
              </div>
