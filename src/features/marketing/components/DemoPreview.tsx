  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <button className="px-3 py-1.5 rounded-md border border-slate-700 text-slate-200 hover:bg-slate-800">
          Start Timer
        </button>
        <button className="px-3 py-1.5 rounded-md border border-slate-700 text-slate-200 hover:bg-slate-800">
          Upload File
        ...
        <details className="relative ml-auto">
          ...
          <div className="absolute right-0 mt-2 w-80 rounded-md border border-slate-700 bg-slate-900 shadow-2xl p-3 text-sm text-slate-200">
            ...
            <p className="text-slate-300">
              All cards are draggable and reorderable. Add or remove widgets anytime to tailor your
              workspace.
            </p>
          </div>
        </details>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-5 lg:col-span-2">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <a
                  key={`demo-cal-${day}`}
                  href={href}
                  className={`relative group block py-2 rounded border border-slate-700 ${isMarked ? 'text-white ring-1 bg-indigo-600/20 ring-indigo-500/40' : 'bg-slate-800/60 text-slate-300'} transition-colors`}
                >
                  {day}
                  {isMarked && (
                    <span
                      aria-hidden
                      className="pointer-events-none absolute top-0.5 right-0.5 h-1.5 w-1.5 rounded-full bg-rose-500"
                    />
                  )}
                </a>
              );
          ...
          <a
            href={`/demo-dashboard/calendar?d=${year}-${pad(month + 1)}-${pad(1)}T09:00`}
            data-calendar-cta
            className="inline-block mt-3 px-3 py-1.5 rounded border border-slate-700 hover:bg-slate-800"
          >
            Open Calendar
          </a>
        </div>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            {['Website Redesign', 'Inventory Module', 'Mobile App'].map((label) => (
              <div key={label} className="flex items-center gap-2">
                ...
                />
                <button className="px-2 py-1 rounded-md border border-slate-700 text-slate-200 hover:bg-slate-800">
                  Save
                </button>
              </div>
            ))}
