...
export default function InventoryClient() {
  ...
  const [importErrors, setImportErrors] = useState<{ line: number; message: string }[] | null>(
    null,
  );
  ...
  useEffect(() => {
    load();
  }, [query, categoryFilter, supplierFilter, statusFilter, page, pageSize, sort.field, sort.order]);
  ...
  async function createItem() {
    ...
    const costCents =
      Math.round(parseFloat((form.cost || '0').replace(/[^\d.\-]/g, '')) * 100) || 0;
    const priceCents =
      Math.round(parseFloat((form.price || '0').replace(/[^\d.\-]/g, '')) * 100) || 0;
    ...
    <div className="space-y-4 pt-0 md:pt-1">
      <div className="flex flex-col md:flex-row gap-2 md:items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <a
            href={`/api/inventory/export?${new URLSearchParams({ computed: '1', q: query, category: categoryFilter, supplier: supplierFilter, status: statusFilter }).toString()}`}
            className="px-3 py-1.5 rounded-md border border-slate-700 hover:bg-slate-800"
          >
            Export CSV
          </a>
            ...
            <tr className="text-left">
              {[
                ...
                <th key={col.key} className="px-3 py-2 select-none">
                  <button
                    className={`inline-flex items-center gap-1 hover:text-slate-200 ${col.key !== 'actions' ? '' : 'cursor-default'}`}
                    disabled={
                      col.key === 'actions' ||
                      col.key === 'status' ||
                      col.key === 'stockValue' ||
                    ...
                    onClick={() => {
                      if (
                        ...
                        return;
                      const next: 'asc' | 'desc' =
                        sort.field === col.key && sort.order === 'asc' ? 'desc' : 'asc';
          ...
          <tbody>
            {filtered.map((i) => {
              ...
              return (
                <tr
                  key={i.id}
                  className={`border-t border-slate-800 ${low ? 'bg-rose-950/30' : ''}`}
                >
                  ...
                  <td className="px-3 py-2">
                    <input
                      ...
                      className="w-full px-2 py-1 rounded bg-slate-900 border border-slate-700"
                      defaultValue={i.sku || ''}
                      onBlur={(e) => updateField(i.id, { sku: e.target.value })}
                    />
                  </td>
                  <td className="px-3 py-2">
                    <input
                      ...
                      onBlur={(e) => updateField(i.id, { categoryName: e.target.value })}
                    />
                  </td>
                  <td className="px-3 py-2">
                    <input
                      ...
                      onBlur={(e) => updateField(i.id, { supplierName: e.target.value })}
                    />
                  </td>
                  <td className="px-3 py-2 font-mono">
                    <input
                      ...
                      placeholder="Qty"
                      type="number"
                      className={`w-24 px-2 py-1 rounded bg-slate-900 border border-slate-700 ${low ? 'text-rose-400' : ''}`}
                      defaultValue={i.quantity}
                      onBlur={(e) => updateField(i.id, { quantity: Number(e.target.value || 0) })}
                    />
                  </td>
                  <td className="px-3 py-2 font-mono">
                    <input
                      ...
                      defaultValue={i.minQuantity}
                      onBlur={(e) =>
                        updateField(i.id, { minQuantity: Number(e.target.value || 0) })
                      }
                    />
                  </td>
                  <td className="px-3 py-2">
                    <input
                      title="Cost"
                      placeholder="Cost ($)"
                      className="w-28 px-2 py-1 rounded bg-slate-900 border border-slate-700"
                      defaultValue={(i.costCents / 100).toFixed(2)}
                      onBlur={(e) =>
                        updateField(i.id, {
                          costCents:
                            Math.round(
                              parseFloat((e.target.value || '0').replace(/[^\d.\-]/g, '')) * 100,
                            ) || 0,
                        })
                      }
                    />
                  ...
                  <td className="px-3 py-2">
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded text-xs border ${low ? (i.quantity <= 0 ? 'border-slate-700 bg-slate-900 text-slate-400' : 'border-amber-600/40 bg-amber-500/10 text-amber-300') : 'border-emerald-700/40 bg-emerald-500/10 text-emerald-300'}`}
                    >
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        onClick={() => updateField(i.id, { quantity: Math.max(0, i.quantity - 1) })}
                      >
                        -
                      </button>
                      <button
                        title="Adjust +1"
                        className="px-2 py-1 rounded border border-slate-700 hover:bg-slate-800"
                        onClick={() => updateField(i.id, { quantity: i.quantity + 1 })}
                      >
                        +
                      </button>
                      {low && i.quantity > 0 && (
                        <button
                          ...
                          onClick={() => updateField(i.id, { quantity: i.minQuantity + 1 })}
                        >
                          Reorder
                        </button>
                      )}
